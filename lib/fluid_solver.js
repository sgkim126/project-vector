...
function FluidSolver(size, accuracy) {
    this.N = size;
...
FluidSolver.prototype = {
    ...
    applyRadialForce: function(x, y, v) {
        ...
        for (var gy = y - size; gy < y + size+1; gy++) {
            for (var gx = x - size; gx < x + size+1; gx++) {
                var vx = gx - x;
                var vy = gy - y;
                var d = Math.abs(vx * vx + vy * vy);
                if (d > 0.2 * 0.2) {
                    d = Math.sqrt(d);
                    vx /= d; vy /= d;
                    var lv = v * (maxDist - d);
                    this.applyForceIntegerPosition(gx, gy, vx * lv, vy * lv);
                }
            }
        }
    },
